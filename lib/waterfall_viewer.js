/**
 * Renders a waterfall graph of Samples from delta and offset key
 * value pairs.  The entire yerf tree is available in the webpage
 * on the yerf() JS object, but this library reconstructs the tree
 * from the key value pairs because it assumes that all it
 * has are the key/value pairs as the results of a DB query or
 * data on a stats dashboard.  This allows this library to be used
 * with data from kivi/yerf and with DBs that aggregate reported data.
 */
...
var TreeRenderer = function (jQuery, $appendTo) {
  ...
  this.renderTree = function (tree, offsets, deltas) {
    var that = this;
    var maxTime = 0;
    _.each(deltas, function (delta, key) {
      var timeSpan = delta + offsets[key];
      if (timeSpan > maxTime) {
        maxTime = timeSpan;
      }
    });
    that.pixelsPerMillisecond = maxTime / this.$(window).width();
  ...
  this.eachBrowser = function (tree, offsets, deltas) {
    ...
    var $browser = that.$('<div />', {
      'class': 'browser'
    , style: 'width:' + this.$(window).width() + 'px;'
    });
  ...
  this.renderChildren = function ($parentElement, tree, keyList, parentOffset, offsets, deltas) {
    ...
    // Sort the children by startedAt, stoppedAt, key
    // Then render them
    _.chain(tree.children)
    .sortBy(function (child) {
      var myKeyList = keyList.slice(0); // Clone a copy of the keylist.
      var entireKey = myKeyList.join('.') + '.' + child.key;
      var delta = that.getVal(deltas[entireKey]);
      var startedAt = child.value;
      var stoppedAt = startedAt + delta;
      ...
      return format(startedAt) + format(stoppedAt) + child.key.toLowerCase();
    })
    .each(function (child) {
      var $node;
      var myKeyList = keyList.slice(0); // Clone a copy of the keylist.
      ...
      if (typeof delta !== 'number') {
        kivi.log('Delta is not a number for ', entireKey);
      }
