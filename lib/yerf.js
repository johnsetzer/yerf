...
yerf = (function () {
  ...
  Sample.prototype.backfill = function(key, startedAt, stoppedAt) {
    ...
    backfill.offset = startedAt - this.startedAt;
  ...
  Sample.prototype.backfillRequest = function(urlPattern, key) {
    ...
    if (publicYerf.usesModernPerf) {
      kivi._.each(performance.webkitGetEntries(), function (entry) {
        ...
        if (matches) {
          ...
          this.backfill(entryKey, entry.startTime, entry.startTime + entry.duration);
        }
      }, this);
    }
